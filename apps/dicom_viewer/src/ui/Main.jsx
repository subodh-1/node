...
export const Main = () => {
  ...
  useEffect(() => {
    function handleResize() {
      ...
      setIsHidden(false); // Set isHidden to true when resize occurs
      setTimeout(() => {
        setIsHidden(true); // Reset isHidden to false after 10 milliseconds
      }, 100);
    }
        ...
        <div ref={drop} style={{ height: '100%' }}>
          ...
          {isReady ? (
            <div className={css.header}>
              ...
              {isReady && !isFullMode && isHidden && (
                <span className={css.header__panels}>
                  <TopToolbar />
                </span>
              )}
              {!isFullMode && isHidden && (
          ...
          {isReady && !isMobile && (
            <div className={cx(isFullMode && css.fullscreen)}>
              ...
              <div className={css.bottleft}>{viewMode === ModeView.VIEW_2D && isHidden && <ZoomTools />}</div>
              <RightPanel />
            </div>
          )}
          {isReady && isMobile && (
