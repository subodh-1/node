...
class Graphics3d extends React.Component {
  ...
  _onWheel(evt) {
    //console.log(`${e.x}, ${e.y}\n`);
    if (this.m_volumeRenderer3D !== null) {
      this.m_volumeRenderer3D.onMouseWheel(evt);
    }
  }
  ...
  onTouchStart(evt) {
    ...
    if (evt.touches.length === 2) {
      this.touching = true;
      this.touchStartDist = this.getDistance(evt.touches[0], evt.touches[1]);
      return;
    }
    const touches = evt.changedTouches;
    const touch = touches[touches.length - 1]; // Get the last touch (assuming single touch)
    const box = this.m_mount.getBoundingClientRect();
    const x = touch.pageX - box.left;
    const y = touch.pageY - box.top;
    if (this.m_volumeRenderer3D !== null) {
      this.m_volumeRenderer3D.onMouseDown(x, y, this.props.ereaseStart);
    }
  }

  onTouchMove(evt) {
    // evt.preventDefault(); // Prevent default touch behavior
    if (this.touching && evt.touches.length === 2) {
      const currentDist = this.getDistance(evt.touches[0], evt.touches[1]);
      const delta = currentDist - this.touchStartDist;
      const wheelEvent = { deltaY: delta }; // Use negative delta to simulate mouse wheel
      this._onWheel(wheelEvent);
      this.touchStartDist = currentDist; // Update the start distance for the next move event
      return;
    }
    const touches = evt.changedTouches;
    const touch = touches[touches.length - 1]; // Get the last touch (assuming single touch)
    ...
    const y = touch.pageY - box.top;
    if (this.m_volumeRenderer3D !== null) {
      this.m_volumeRenderer3D.onMouseMove(x, y, this.props.ereaseStart);
    }
  }
