...
class ToolDistance {
  ...
  onTouchStart(xScr, yScr, store) {
    ...
    // Create a start point for the line
    const vStart = {
      x: vTex.x,
      y: vTex.y,
    };

    // Create an end point for the line (initially same as start point)
    const vEnd = {
      x: vTex.x,
      y: vTex.y,
    };
    ...
    // Push the line object into the array of lines
    this.m_lines.push(objLine);
  ...
  onTouchMove(xScr, yScr, store) {
    ...
    // Update the end point of the last line to the current touch position
    const vEnd = {
      x: vTex.x,
      y: vTex.y,
    };
    ...
    if (numLines > 0) {
      ...
      // Update the end point of the last line
      objLastLine.ve = vEnd;

      // Calculate the distance between start and end points
      objLastLine.distMm = this.getDistMm(objLastLine.vs, objLastLine.ve);

      // If the distance is greater than a minimum threshold, update the rendering
      const MIN_DIST_MM = 0.00001;
      if (objLastLine.distMm > MIN_DIST_MM) {
        // Invoke render event
        this.m_objGraphics2d.forceUpdate();
  ...
  onTouchEnd() {
    ...
    if (numLines > 0) {
      ...
      // If the distance is less than or equal to a minimum threshold, remove the last line
      const MIN_DIST_MM = 0.00001;
      if (objLastLine.distMm <= MIN_DIST_MM) {
