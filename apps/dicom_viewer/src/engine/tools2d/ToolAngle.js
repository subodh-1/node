...
class ToolAngle {
  ...
  //SUBODH:: touch events
  // SUBODH::Translate onMouseDown to onTouchStart for drawing angles with touch events
  onTouchStart(xScr, yScr, store) {
    // Convert touch coordinates to texture coordinates
    const vTex = ToolDistance.screenToTexture(xScr, yScr, this.m_wScreen, this.m_hScreen, store);

    // If this is the first touch, assign all 3 points at the same place
    if (this.m_numClicks === 0) {
      for (let i = 0; i < 3; i++) {
        this.m_points[i].x = vTex.x;
        this.m_points[i].y = vTex.y;
    ...
    // If this is the second touch, increment the click count
    else if (this.m_numClicks === 1) {
      this.m_numClicks++;
    }
    // If this is the third touch, finalize the angle
    else {
      console.log('3rd touch: finalize angle');
      // Extract coordinates for each of the three points
      const v0 = {
        x: this.m_points[0].x,
        y: this.m_points[0].y,
      };
      ...
      // Calculate the angle between the lines formed by the three points
      const ang = this.getAngle();

      // Create an object representing the angle with its points and value
      const objAngle = {
        points: [v0, v1, v2],
        angle: ang,
      };
      // Add the angle object to the array of angles
      this.m_angles.push(objAngle);
      // Reset the click count for the next angle measurement
      this.m_numClicks = 0;
    }
  }
  // SUBODH::Translate onMouseMove to onTouchEnd
  ...
  // SUBODH::Translate onMouseUp to onTouchEnd
  onTouchEnd(xScr, yScr, store) {
    console.log(xScr);
    console.log(yScr);
    console.log(store);
    return;
